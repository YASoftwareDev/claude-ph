...
A two-file Claude Code tool that searches the user's **full** prompt history
...
1. Create the target directories (idempotent):
   ...
   ```
2. Copy the two artifacts from this repo to their install locations:
   ```sh
   cp ph.py ~/.claude/scripts/ph.py
   cp ph.md ~/.claude/commands/ph.md
   ```
3. Verify both files copied (non-empty, exit status 0).
4. Tell the user to **restart Claude Code** so the new `/ph` command is loaded,
   then they can run e.g. `/ph deploy`.
...
```sh
rm -f ~/.claude/scripts/ph.py ~/.claude/commands/ph.md
```
